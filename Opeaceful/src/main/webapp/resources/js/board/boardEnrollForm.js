...
////////////////////////////////////////////////
 ...
 // 혜린 보거라 => returnFormData함수를 폼태그 서브밋 되기 전에 실행될 수 있도록
//구글링으로 이벤트 처리 찾고 적용시키기
 ...
 export function returnFormData(tinyId) {
     ...
     if (imgSrc.startsWith('blob')) {
       for (let i in fileList) {
         ...
         if (fileList[i].src == imgSrc) {
           ...
           img.src = dataTransfer.files.length - 1;
   ...
   //img(input요소 아이디 히든시킨거)
   // 정보 뽑아내기 끝나면 다음을 위해 파일리스트 비우기
   resetInputFileList();
 
   return stateimg;
 }
//let a = returnFormData(tinyId);

  // 혜린 폼태그 전송전에 실행시켜야하는 함수
// tinymce.triggerSave();  returnFormData(tinyId);
// stateimg 꼭 담긴 값 가지고 백으로 보내기(수정 전 후 똑같이 유지된 이미지 담겨있음)
// 백에서는 해당 게시글 번호에 stateimg(changeName)랑 똑같은게 있는지 유무를 확인하고,
// 그 나머지 해당안되는 이미지이름들은 지워주기()
// FileRenamePolicy (Util이랑 똑같은거.)
