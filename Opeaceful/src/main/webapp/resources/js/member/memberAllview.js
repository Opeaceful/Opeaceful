...
function memberSelectAjax() {
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (
        let i = pagination.startPage;
        i <= Math.min(pagination.endPage, pagination.maxPage);
        i++
      ) {
        phtml += ` <li class="page-item"><button class="page-link" data-page="${i}">${i}</button></li>`;
      }
...
function pitem() {
  ...
  const selectBtn = document.querySelector(
    `.page-link[data-page='${pagination.currentPage}']`
  );
...
function memberUpdaetajax(id) {
  $.ajax({
    ...
    success: function (result) {
      ...
      document.querySelector("input[name=extension]").value =
        result.m.extension;
      document.querySelector("input[name=phone]").value = result.m.phone;
      document.getElementById("join-date").value = result.m.ShireDate;
      document.querySelector("input[name=annualLeaveCount]").value =
        result.m.annualLeaveCount;
      LeaveDate.value = "";
      ...
      for (let i = 2; i < addressList.length; i++) {
        document.getElementById("user-address-dtail").value +=
          "," + addressList[i];
      }
...
LeaveDate.addEventListener("change", function () {
  ...
  if (inputLeaveDate) {
    ...
    swal(
      inputLeaveDate + " 퇴사일 설정",
      `퇴사일 설정시 해당 직원은 변경할 수 없습니다. 해당 직원은 3년뒤 자동 삭제 처리됩니다.`,
      {
        buttons: { confirm: "확인", cancel: "취소" },
      }
    ).then(function (isConfirm) {
      if (isConfirm) {
        // 확인선택시 퇴사자로 변경 : input박스 수정불가!
...
document
  .getElementById("member-btn-close")
  .addEventListener("click", inputChangeable);
