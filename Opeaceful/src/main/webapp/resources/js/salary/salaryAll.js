...
function allSalaryTable() {
    //만들어진 tr태그에 이벤트 부여
    const AllsalaryTableTr = document.querySelectorAll('#Am-salary-table tr[data-id]');
...
function deleteTableClick() {
    const deleteTablebtn = document.querySelectorAll('#table-form-salary td button');
...
function tataldiv(tableid){
    ...
    for (let i = 0; i < trs.length; i++) {;
        if(trs[i].firstElementChild.textContent){
            payments += trs[i].firstElementChild.textContent+','+trs[i].querySelector('input[type="number"]').value+','
        }else{
            payments += trs[i].querySelector('input[type="text"]').value+','+trs[i].querySelector('input[type="number"]').value+','
        }
