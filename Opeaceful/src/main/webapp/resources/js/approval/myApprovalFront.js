...
let extraAprModalEvent = function () {
  ...
  document
    ...
    .addEventListener("click", () => {
      ...
      /**
       * `LINE_NO`, `USER_NO`, `TITLE`
       * `LEVEL`, `LINE_NO`, `USER_NO`, `TYPE`
       */
      ...
      // 나중에 서버에 넘겨줄 객체
      let favorLine = { title: favorTitle, lineList: [] };
      ...
      liList.forEach((li, index) => {
        ...
        let line = { level: level, type: type, userNo: userNo };

        favorLine.lineList.push(line);
      });

      console.log(favorLine);

      // 만약 선택된 결재자 없으면 그냥 종료
      if (favorLine.lineList.length <= 0) {
