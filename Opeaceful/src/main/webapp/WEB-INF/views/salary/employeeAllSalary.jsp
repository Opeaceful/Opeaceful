...
<body>
	...
	<c:if test="${empty map.no}">
		<c:set var="url"
			value="AllSalary?year=${map.year}&month=${map.month}&team=${map.team}&cpage=" />
	</c:if>
	<c:if test="${not empty map.no}">
		<c:set var="url"
			value="AllSalary?year=${map.year}&month=${map.month}&no=${fn:join(map.no,',')}&cpage=" />
	</c:if>
	...
	<div class="content-wrap">
		...
		<div class="container">
			...
			<div class="d-inline-flex">
				...
				</select> <select class="form-select form-select-sm mb-3 ms-1"
					id="salary-year" name="year">
				</select> <select class="form-select form-select-sm mb-3 ms-1"
					id="salary-month" name="month">
					<option value="13">전체</option>
				</select>
				<div class="search-wrap2 input-group mb-3 ms-1">
					<input type="text"
						class="search-input2 form-control box-shadow-none"
						id="member-search-keyword" placeholder="사원이름 입력">
					<button type="button" class="btn btn-outline-secondary search-btn2"
				...
				<tbody class="table-group-divider" id="Am-salary-table">
					<c:forEach items="${LSalry}" var="salry">
						<tr data-id="${salry.getSalaryNo()},${salry.getUserName()}">
							<td><c:if test="${salry.getEno() == 0}">
					퇴사자
				</c:if> <c:if test="${salry.getEno() != 0}">
					${salry.getEno()}
				</c:if></td>
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination">
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="${url}${pi.currentPage -1}"><span aria-hidden="true">&laquo;</span></a></li>
							</c:otherwise>
						</c:choose>

						<c:forEach var="item" begin="${pi.startPage }" end="${pi.endPage}">
							<li class="page-item"><a class="page-link"
								href="${url}${item}" id="CP${item}">${item }</a></li>
						</c:forEach>
						...
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="${url}${pi.currentPage + 1 }"><span
			...
			<!-- Modal : 수정용 -->
			<div class="modal fade" id="salaryModal" data-bs-backdrop="static"
						...
						<div class="modal-body scroll-bar">
							<div class="container">
								...
								<div class="salary-btn">
									<button type="button" class="btn btn-primary ms-1 ok-common "
										id="salary-update">수정</button>
									<button type="button"
										class="btn btn-danger  ms-1 cancel-common" value=0
										id="salary-delete">삭제</button>
								</div>

								<h1 class="text-center" id="salaryModalTitle"></h1>

								<br>
								<div class="row f-detween" id="salryUserData"></div>
